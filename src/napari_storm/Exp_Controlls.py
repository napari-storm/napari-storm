...
class RightClickPaninng(QWidget):
    ...
    def move_anker(self):
        ...
        self.viewer.layers["anker"].data = (
            np.reshape(self.anker_coords + center, (6, 3)),
            self.anker_faces,
            np.ones(6),
        )
    ...
    def right_click_pan(self):
        # self.copy_on_mouse_press =
        # self.viewer.window.qt_viewer.on_mouse_press
        ...
        def our_mouse_move(event: QtGui.QMouseEvent) -> None:
            if not self.mouse_down:
                return
            event.blocked = True
            # print("mouse move", event.native.x(), event.native.y(),
            # event.native.button())
        ...
        def our_mouse_release(event=None):
            if event.type == "mouse_release":
                if event.button == 2:
                    if not self.mouse_down:
                        return
                    # print("mouse release", event.native.x(),
                    # event.native.y(), event.native.button())
                    self.mouse_down = False
                    self.start_x = event.native.x()
                    self.start_y = event.native.y()
                    self.remove_anker()
    ...
    def _handle_move(self, x, y):
        ...
        rx = delta_x * np.cos(alpha) * np.cos(beta) + delta_y * (
            -np.sin(alpha)
        ) * np.cos(beta) * np.sin(gamma)
        ry = (
            delta_x * np.sin(alpha) * np.cos(beta) * np.sin(gamma)
            + delta_x * np.cos(alpha) * np.sin(beta)
            + delta_y * np.cos(alpha) * np.cos(beta) * np.sin(gamma)
        )
        rz = -delta_x * np.sin(alpha) * np.cos(gamma) - delta_y * np.cos(
            alpha
        ) * np.cos(gamma)
...
class MouseControlls(QWidget):  # Experimental Fly through mode
    ...
    def _handle_moveL(self, x, y):
        ...
        z, y, x = self.viewer.camera.center
        y += (
            np.cos(2 * 3.14145 * gamma / 360)
            * self.viewer.window.qt_viewer.height()
            * 0.05
        )
        x -= (
            np.sin(2 * 3.14145 * beta / 360)
            * self.viewer.window.qt_viewer.width()
            * 0.05
        )
        self.viewer.camera.center = (z, y, x)
        self.viewer.camera.angles = (alpha, beta, gamma)
    ...
    def _activate(self):
        ...
        def our_mouse_press(event=None):
            # print("mouse press", event.native.x(), event.native.y(),
            # event.native.button())
            self.mouse_down = True
            self.start_x = event.native.x()
            self.start_y = event.native.y()
