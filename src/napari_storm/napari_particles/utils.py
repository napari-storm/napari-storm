...
def generate_billboards_2d(
    ...
    """
    verts0 = np.array([[-0.5, -0.5], [0.5, -0.5], [0.5, 0.5], [-0.5, 0.5]]).astype(
        np.float32
    )
    ...
    texcoords0 = np.array([[0, 0], [1, 0], [1, 1], [0, 1]]).astype(np.float32)

    texcoords = np.tile(texcoords0, (n, 1))

    faces = np.tile(np.array([[0, 1, 2], [0, 3, 2]]), (n, 1))
    faces = faces + np.repeat(
        np.repeat(4 * np.arange(n)[:, np.newaxis], 3, axis=-1), 2, axis=0
    )
