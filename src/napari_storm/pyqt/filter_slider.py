...
class RangeSlider3(QDoubleRangeSlider):
    ...
    def reset(self):
        # Reset is also used while unloading the final dataset.  Suppress the
        # feedback callback until a dataset/parameter pair exists again.
    ...
    def mouseReleaseEvent(self, ev):
        super().mouseReleaseEvent(ev)  # release knob tracking in parent widget
        self.data_filter_itf.filter_slider_values_decimal = (
            np.asarray(self.value()) / self.Range
        )

    def virtual_feedback(self):
        if (
            self.data_filter_itf is None
            or not self.data_filter_itf.list_of_datasets
            or not self.data_filter_itf.list_of_filterable_parameters
        ):
            return
        if self.ax.lines:
        ...
        tmp_max = np.max(
            getattr(
                self.data_filter_itf.list_of_datasets[
                    self.data_filter_itf.current_dataset_idx
                ].locs_active,
...
class RangeSlider4(QDoubleRangeSlider):
    ...
    def mouseReleaseEvent(self, ev):
        super().mouseReleaseEvent(ev)  # release knob tracking in parent widget
        self.data_filter_itf.xrange_slider_values_decimal = (
            np.asarray(self.value()) / self.Range
        )
