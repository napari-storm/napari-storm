...
class DataToLayerInterface:  # localization always with z # switch info with channel controlls #
    ...
    def create_remove_grid_plane_state(self, enable):
        if enable:
            ...
            default_line_dist_nm = 1000
            num_of_lines_x = int(np.floor((self.render_range_x[1] - self.render_range_x[0]) / default_line_dist_nm))
            num_of_lines_y = int(np.floor((self.render_range_y[1] - self.render_range_y[0]) / default_line_dist_nm))
            ...
            self.grid_plane_layer = self.viewer.add_vectors(vectors, edge_width=default_line_thickness_nm,
                                                            name="Grid_Plane", edge_color='white')
        else:
    ...
    def update_grid_plane(self, z_pos=None, line_thickness=None, line_distance_nm=None):
        if line_distance_nm:
            ...
            line_thickness_nm = self.grid_plane_layer.edge_width
            self.viewer.layers.remove('Grid_Plane')
            num_of_lines_x = int(np.floor((self.render_range_x[1] - self.render_range_x[0]) / line_distance_nm))
            num_of_lines_y = int(np.floor((self.render_range_y[1] - self.render_range_y[0]) / line_distance_nm))
